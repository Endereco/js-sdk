...
const EnderecoIntegrator = {
    ...
    initAMS: async(
        ...
        // Preselect a value.
        if (!addressObject.countryCode && integrator.defaultCountrySelect) {
            addressObject.countryCode = integrator.defaultCountry;
        }

        if (!!options.addressType) {
            addressObject.addressType = options.addressType
        }
    ...
    addSubmitListener: (form, dataObject, integrator) => {
        if (!integrator.formSubmitListeners[form]) {
            integrator.formSubmitListeners[form] = [];
            attachSubmitListenersToForm(form);
            // Add diverse event listeners to the form.
        }
        integrator.formSubmitListeners[form].push(dataObject)
    },
