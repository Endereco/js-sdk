import notFoundFlag from './templates/flags/not_found.svg'
...
class MappingService {
  ...
  /**
   * Get the country code (ISO code) for a given country ID.
   * @param {string} countryId - The ID of the country from the system.
   * @returns {string} - The country ISO code corresponding to the given country ID, or the country ID if no mapping is found.
   */
  getCountryCode (countryId) {
    const country = this.mappings.country.find(c => c.id === countryId)
  ...
  getCountryId (countryCode) {
    ...
    const country = this.mappings.country.find(c => c.countryCode === countryCode)
   ...
   * @param {string} countryCode - The ISO code of the country.
   * @returns {string|null} - The country name corresponding to the given country code, or null if no mapping is found.
   */
  getCountryName (countryCode) {
    countryCode = this.#normalizeCountryCode(countryCode)
    const country = this.mappings.country.find(c => c.counryCode === countryCode)
  ...
  getSubdivisionCode (subdivisionId, countryCode = null) {
    ...
    return this.mappings.subdivision.find(
      s => s.id === subdivisionId && (!normalizedCountryCode || s.countryCode === normalizedCountryCode)
    )?.subdivisionCode ?? subdivisionId
  }
  ...
  getSubdivisionId (subdivisionCode, countryCode = null) {
    const normalizedCountryCode = this.#normalizeCountryCode(countryCode)
    return this.mappings.subdivision.find(
      s => s.subdivisionCode === subdivisionCode && (!normalizedCountryCode || s.countryCode === normalizedCountryCode)
    )?.id ?? subdivisionCode
  }
